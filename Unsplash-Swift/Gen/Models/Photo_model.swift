...
public struct Photo: _InitializableFromJSON, _EncodableToJSON {
    ...
    public init(json: JSON) throws {
        ...
        self.location = try json["location"].getOptionalObject(Location)
        self.exif = try json["exif"].getOptionalObject(Exif)
    }
    ...
    public func toJSON() -> JSON {
        ...
        if let value = self.location {
            json["location"] = value.toJSON()
        } else {
            json["location"] = JSON(NSNull())
        }
        if let value = self.exif {
            json["exif"] = value.toJSON()
        } else {
            json["exif"] = JSON(NSNull())
        }
        return JSON(json)
    }
}
